...
async function open(browser, H) {
  ...
  const ok = await page.evaluate(() => document.fonts.check('40px Pop') && document.fonts.check('40px Dela'));
...
(async () => {
  ...
  if (mode === "stills") {
    for (const H of [1920, 1350]) await stills(browser, H, [1.2, 4.4, 7.2, 8.6, 11.8, 13.6, 16.6, 18.4]);
  } else {
    await video(browser, 1920, "mori-kuma_9x16.mp4");
    await video(browser, 1350, "mori-kuma_4x5.mp4");
  }
  await browser.close();
