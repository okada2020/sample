...
0. **範囲指定（ROI）** — 読みたい部分だけを枠で囲うと、周囲の余計な文字を拾わなくなり、その部分の実効解像度も上がる。設定で「読み取り前に範囲を指定」を有効にするか、結果画面の共有メニューから後がけできる。
...
3. **影・照明ムラの平坦化** — 大きくぼかした自分自身で元画像を割る。ぼかした画像は「その場所の紙の明るさ」に相当するので、割ると影や照明ムラだけが打ち消される。片側に影が落ちた書類や、蛍光灯が映り込んだ紙に効く。
4. **輪郭の強調** — 控えめなコントラスト補正 + アンシャープマスク。かけすぎると細い線が飛ぶので弱め。
5. **向きの自動判定** — 0/90/180/270 度を `.fast` で試し、`信頼度 × 文字数` の合計が最大の向きを選んでから、本命の `.accurate` を 1 回だけ走らせる。横向きに撮った写真が丸ごと読めない事故を防ぎます。
6. **読み順の復元** — Vision は必ずしも読み順に返さないため、まず行（縦書きでは列）にクラスタリングしてから、その中を整列して並べ直す。縦書きは右の列から左へ。さらに矩形の形状（縦長の断片が 7 割以上）から縦書きらしさを自動判定して読み順を切り替えます（設定で無効化可）。
7. **日本語の空白除去** — 全角文字どうしに挟まれた空白だけを削除（「東京 都」→「東京都」、`Tokyo Station` はそのまま）。

8. **撮影距離の警告** — 行の高さの中央値がピクセル単位で 24 を下回ると、結果画面に警告と対処（近づく／範囲を指定する）を表示する。Vision は文字高が 32 ピクセル以上あると安定するため、ここが足りていないと後処理では埋められない。
9. **定型フィールドの桁補正** — 日付・電話番号・郵便番号・金額に限り、`O↔0`、`l↔1`、`S↔5` といった典型的な取り違えを数字へ寄せる。英単語を壊さないよう、「〒 が付く」「0 で始まる」「年月が付く」「¥ が付く」といった強い手がかりがある箇所だけを対象にする。

### 時間と引き換えに精度を取るもの（設定でオン）

- **分割認識** — Vision は内部で大きな画像を縮小するため、細かい文字を取りこぼす。画像を 12% ずつ重ねながら 2×2 または 3×3 に分割して認識し、重複を除いて結合すると、実効解像度が上がって小さい文字が読めるようになる。細かい表や注釈の多い書類向け。
- **言語補正あり・なしの突き合わせ** — 両方で認識し、行ごとに信頼度の高い方を採用する。文章は補正あり、型番や数字の羅列は補正なしが強いので、二択を自動で使い分けられる。採用しなかった方は候補として残る。
- **連写して多数決** — シャッター 1 回で 3〜5 枚撮り、行ごとに多数決を取る。カメラの手ブレによる誤認識に強くなる。
...
└── PreciseOCR/
    ...
    │   ├── TextNormalizer.swift     # 定型フィールドの桁補正
    │   ├── OCRSettings.swift
...
- 結果画面に「文字が小さすぎます」と出たら、近づいて撮り直すか、範囲を指定して読み直す
- 細かい表や注釈が読めないときは、設定の「細かい文字を分割して読む」を有効にする
