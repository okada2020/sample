...
(() => {
    ...
    if (B && !B.dead) {
      ...
      if (B.fight) {
        ...
        if (B.rollT <= 0) {
          ...
          B.rollT = iv;
          const r = crowdRadius(c.count);
          // 山の後ろから湧いて、山を抜けてクマへ転がっていく（切れ目なく流れ続けるように）
          S.fx.push({ kind: 'roll', x: c.x + rnd(-r, r), z: c.z - r - rnd(0.4, 2.6), y: 0.3,
  ...
  function render() {
    ...
    if (c.count > 0) {
      ...
      for (let i = 0; i < n; i++) {                        // 一粒ずつ少しずれて跳ねる（隊列に見えないように）
        const ph = i * 1.7;
        list.push({ type: 'fruit', i: i,
                    x: c.x + off[i][0] + Math.sin(S.clock * 3.1 + ph) * 0.08,
                    z: c.z + off[i][1] + Math.cos(S.clock * 2.6 + ph) * 0.12,
                    y: Math.abs(Math.sin(S.clock * 6.5 + ph)) * 0.10 });
      }
    }
    const DD = S.death;
