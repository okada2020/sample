...
(() => {
  ...
  function update(dt) {
    ...
    // ── 本編：転がる果物がプレイヤー
    S.time += dt;
    const speed = 23 + Math.min(6, c.z * 0.012);        // 坂なので少しずつ速くなる
    ...
    if (B && !B.dead) {
      ...
      if (c.z > B.z - 40) B.z += 2.2 * dt;                 // にじり寄ってくる
      if (c.z >= B.z - 3.2) {
        const dmg = Math.min(c.count, B.hp);
        c.count -= dmg; B.hp -= dmg;
        B.hurt = 1; S.shake = 1; S.flash = 0.4;
        pop('-' + dmg, '#fff1a8', 0, B.z - 2, 4);
        puff(0, B.z - 2, 12, true); burstFruit(0, B.z - 3, 8);
        sfx(120, 0.3, 'sawtooth', 0.07, 60);
        if (B.hp <= 0) {
          B.dead = 0.001; S.killed++;
          pop('ボス撃破！', '#ffd166', 0, B.z, 5);
          sfx(523, 0.15, 'square', 0.07); setTimeout(() => sfx(880, 0.25, 'square', 0.07), 140);
        } else if (c.count <= 0) {
          return gameOver('ボスに押し負けた…');
        }
      }
    }
    if (B && B.dead) { B.dead += dt; B.z += 30 * dt; }
    if (c.z >= S.lane) goal();
  }
    ...
    for (const o of list) {
      ...
      if (o.type === 'tree') drawTree(p, p.s * o.h * 0.95);
        ...
        if (e.dead) {
          ...
          if (e.max > 1) {                           // 頭上に HP バーと残り数
            const fs = clamp(p.s * 0.5, 11, 28);
            const by = p.y - d.h * p.s - fs * 0.5;
            const bw = clamp(p.s * d.w * 1.6, 26, 120), bh = Math.max(5, fs * 0.28);
            ctx.fillStyle = '#1b1b1f';
            ctx.fillRect(p.x - bw / 2 - 2, by + fs * 0.62 - 2, bw + 4, bh + 4);
            ctx.fillStyle = '#5a1f1f';
            ctx.fillRect(p.x - bw / 2, by + fs * 0.62, bw, bh);
            ctx.fillStyle = '#e33b2e';
            ctx.fillRect(p.x - bw / 2, by + fs * 0.62, bw * clamp(e.hp / e.max, 0, 1), bh);
            worldText(e.hp, p.x, by, fs, '#fff');
          }
        }
      }
    }
