# ぱんがじうす紹介動画 — カット構成と画像プロンプト

10秒 / 9:16（1080x1920, 30fps）。フラットでかわいいイラスト5枚を紙芝居のように
横スライドでつなぐ。1カット2.24秒表示 + 0.3秒スライドで合計10.0秒。

画像は Higgsfield の `nano_banana_pro`（2K, **3:4**）で生成した。
3:4 はカード内のイラスト枠と同じ比率なので、トリミングで絵が欠けない。

文字は画像に焼き込まず、`build_kamishibai.py` 側で日本語フォントで描画している
（画像モデルに日本語を書かせると崩れるため）。
