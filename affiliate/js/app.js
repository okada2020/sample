...
(function () {
  ...
  function initSimulator() {
    ...
    sim.addEventListener("click", function (e) {
      ...
      if (!b) return;
      $$("button[data-choice]", sim).forEach(function (x) {
        x.setAttribute("aria-pressed", String(x === b));
      });
      var raw = b.dataset.choice;
      var n = isNaN(Number(raw)) ? raw : Number(raw);
      $("[data-sim-out]", sim).innerHTML = R.simResults(n);
      track("simulator_change", { choice: String(n) });
    });
  }
  ...
  function initFilters() {
    ...
    filters.addEventListener("click", function (e) {
      ...
      $$(".cmp tbody tr", box).forEach(function (tr) {
        var hit = !g || (tr.dataset.filters || "").split("|").indexOf(g) !== -1;
        tr.hidden = !hit;
        if (hit) shown++;
      });
